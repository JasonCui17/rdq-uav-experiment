...
from rdq_uav.lidar_v1 import CandidateLoss,CandidateSelector,LiDARUAVDetector
from rdq_uav.lidar_v1.geometry import HierarchyBuilder
from rdq_uav.lidar_v1.model import RelativeBias,SpatialBlock,local_groups,morton_order

CFG=yaml.safe_load((ROOT/"configs/lidar_uav_v1.yaml").read_text())
...
def test_architecture_contract_and_initialization():
    ...
    source=(ROOT/"src/rdq_uav/lidar_v1/model.py").read_text().lower();assert "attentionpool" not in source and "dbscan" not in source and "occupancy_mask" not in source
