#!/usr/bin/env python3
"""Evaluate V1 checkpoint and optionally export synchronized candidate features."""
...
from rdq_uav.lidar_v1.runtime import evaluate_batch,move_batch,summarize_metrics

def main():
    ap=argparse.ArgumentParser();ap.add_argument("checkpoint",type=Path);ap.add_argument("--config",type=Path,default=ROOT/"configs/lidar_uav_v1.yaml");ap.add_argument("--split",choices=["train_sub","validation_sub","heldout_test_sub"],default="validation_sub");ap.add_argument("--output",type=Path,required=True);ap.add_argument("--export-candidates",action="store_true");ap.add_argument("--max-samples",type=int);args=ap.parse_args()
