#!/usr/bin/env python3
"""Formal lidar_uav_v2 training CLI with strict train/validation GT provenance."""
...
from rdq_uav.lidar_v2 import (CandidateLoss,CandidateSelector,LiDARUAVDataset,
    LiDARUAVDetector,LiDARUAVValidationDataset,collate_lidar_samples)
from rdq_uav.lidar_v2.runtime import UpdateScheduler,move_batch,optimizer_groups
from rdq_uav.lidar_v2.training import (EpochTracker,append_csv,finite_or_raise,
...
def parse_args():
    ...
    parser.add_argument("--config",type=Path,default=ROOT/"configs/lidar_uav_v2.yaml")
...
def resolve_device(text):
    ...
    if "," in text or int(os.environ.get("WORLD_SIZE","1"))>1:
        raise RuntimeError("Reliable DDP is not implemented in this V2-base CLI; use one CUDA index")
...
def print_run_header(args,cfg,model,device,epochs,batch_size,accum,workers):
    ...
    print("LiDAR UAV Transformer V2-base\n"+"-"*60)
...
def main():
    ...
    if int(os.environ.get("WORLD_SIZE","1"))>1:raise RuntimeError("DDP is not enabled/tested for lidar_uav_v2 formal training")
    ...
    out=(args.output or (args.resume.parent if args.resume else ROOT/"outputs/own_multimodal_research/lidar_uav_v2"/default_name)).resolve()
    ...
    if not smoke:
        report=f"""# LiDAR UAV V2-base Formal Training Report
