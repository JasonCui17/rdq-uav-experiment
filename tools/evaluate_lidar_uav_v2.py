...
def main():
    ...
    cfg=yaml.safe_load(a.config.read_text());device=torch.device('cpu' if a.device=='cpu' else f'cuda:{a.device}')
    ...
    loader=DataLoader(ds,batch_size=1,shuffle=False,num_workers=a.num_workers,collate_fn=collate_temporal_queries)
    metrics,rows,health=validate(model,loader,QueryCausalLoss(cfg),CandidateSelector(cfg),device,
