...
@torch.no_grad()
def validate(model,loader,criterion,selector,device,amp_enabled=False,export_dir=None,checkpoint_path=None):
    ...
    """
    model.eval();rows=[];candidates=[];features=[];spatial_sum=cls_sum=reg_sum=temp_sum=0.;ns=nt=0
    ...
    for raw in tqdm(loader,desc='VAL',dynamic_ncols=True):
        ...
        with torch.autocast(device_type=device.type,dtype=torch.bfloat16,enabled=amp_enabled):out=model(batch)
...
def save_checkpoint(path,model,optimizer,scheduler,epoch,global_step,best,cfg):
    torch.save(dict(model_state=model.state_dict(),optimizer_state=optimizer.state_dict(),scheduler_state=scheduler.state_dict(),
        epoch=epoch,global_step=global_step,best_metric=best,resolved_model_config=cfg,data_config=cfg['data'],
        loss_config=cfg['loss'],selector_config=cfg['selector'],denoise=False),path)
